...
// Inversion bit value at position index in array
void bit_inversion(char *array, int index) {
  char b = array[index / 8];
  char m = (1 << (index % 8));
  array[index / 8] = (b ^ m);
}
...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].child_alloc_xor, NBLK(k) / 2);
    if (k > 0) {
      printf("  split:");
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_inversion(bd_sizes[k].child_alloc_xor, blk_index(k, p) / 2);
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_inversion(bd_sizes[k - 1].child_alloc_xor, blk_index(k - 1, p) / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_inversion(bd_sizes[k].child_alloc_xor,
                  bi / 2);  // change alloc_xor with buddy

    if (bit_isset(bd_sizes[k].child_alloc_xor, bi / 2)) {
      break;  // if alloc_xor with buddy == 1, break out of loop
    }
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      bit_inversion(bd_sizes[k].child_alloc_xor, bi / 2);
    }
  }
}
...
int bd_initfree_pair(int k, int bi, int is_left, int flag) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset(bd_sizes[k].child_alloc_xor, bi / 2)) {
...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {  // skip max size
    ...
    free += bd_initfree_pair(k, left, 1, flag);
    if (!flag) continue;
    free += bd_initfree_pair(k, right, 0, flag);
  }
  return free;
}
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    bd_sizes[k].child_alloc_xor = p;
    memset(bd_sizes[k].child_alloc_xor, 0, sz);
    p += sz;
  }
