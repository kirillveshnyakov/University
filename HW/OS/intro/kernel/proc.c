...
int reg(int pid, int register_num, uint64 *return_value) {
  ...
  if (p >= &proc[NPROC] || p->pid != pid) {
    return -2;
  }
  if (p != curproc && p->parent != curproc) {
    release(&p->lock);
    return -1;
  }

  struct trapframe *tf = p->trapframe;
  uint64 *registers = &tf->s2;
  result = registers[register_num - 2];

  release(&p->lock);

  if (copyout(curproc->pagetable, *return_value, (char *)&result,
              sizeof(result)) < 0) {
    return -4;
  }
